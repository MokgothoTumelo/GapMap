...
- **Runtime.** Browser via the Firebase AI Logic Web SDK (`firebase/app` +
  ...
  process. The live implementation uses `gemini-3.5-flash` as the
  primary and `gemini-3.5-flash-lite` as a capacity fallback; transient
  provider failures are retried and then surfaced if the pool is exhausted.
...
- The agent panel (`frontend/agentPanel/`) now uses the request-scoped
  ...
  configuration and provider errors are shown in the panel. If the app team
...
- `frontend/js/firebase-agent.js` contains the SDK-injection seam for a named
  ...
  Path → Practice → Retest) remains independent of a live model.
- The Companion has a **tool-calling interface** (see
  ...
  tools `getGapMap`, `getLearningPath`, `getConcept`, `getItem`, and
  `getAttemptHistory` are wired end-to-end against the mock `learnerStore`
  (`generateAssessment` is planned). The Gemini 3 thought-signature
