...
Diagnostic / Practice / Retest artefacts without the Learner's context. The
...
- `generateAssessment({ type, targetConcept, itemCount })` — generates and
  persists a Diagnostic / Practice / Retest via `createAssessmentForLearner`
...
- **`generateAssessment` design (settled 2026-08-20):**
  ...
  - **Grounding.** The generated Assessment is grounded in the Learner's
    ...
    Concepts into the generator's prompt; Practice/Retest target the Concept
...
- The e2e vision: the Companion produces arbitrary data for the Learner and the
  ...
  Practice → Retest) remains independent of live AI and can opt out with
  `?agent=off`.
