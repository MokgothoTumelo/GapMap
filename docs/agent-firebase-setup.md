...
tooling and future bundling. Live inference uses `gemini-3.5-flash` first,
then `gemini-3.5-flash-lite` after transient capacity failures; if the pool is
exhausted the error is surfaced in the panel (no fabricated reply).
